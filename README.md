...
1. Create your own configuration directory within the repository folder.

2. Create an `edx.properties` file inside the `OpenEdXMobile` directory of `edx-app-android`. In this `edx.properties` file, set the `edx.dir` property to the path to your configuration directory relative to the `OpenEdXMobile` directory. For example, if I stored my configuration side by side with the `OpenEdXMobile` folder at `my_config` then I'd have the following `edx.properties`:
