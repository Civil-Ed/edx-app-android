...
class CourseRepository @Inject constructor(
    ...
    fun fetchEnrolledCourses(
        type: CourseViewModel.CoursesRequestType,
        callback: NetworkResponseCallback<EnrollmentResponse>
    ) {
        val call = when (type) {
            LIVE -> courseAPI.enrolledCourses
            CACHE -> courseAPI.enrolledCoursesFromCache
            STALE -> courseAPI.enrolledCoursesWithoutStale
        }
